...
//#include <Adafruit_MCP23017.h> //library for HMI
//#include <Adafruit_RGBLCDShield.h> //library for HMI
#include <Process.h>
...
//=====================================================================================================================================================================================================================================================
void loop(void) {
  writeToDB("softanalog", "low_level", low_level);
  writeToDB("softanalog", "high_level", high_level); 
...
//Adjust low level based on average temperature
  if (tempAvg >= 5 && low_level == 30) {
    low_level -= 10;
  } else {
   if (tempAvg < 5 && low_level == 20) {
     low_level += 10;
   }
  }
  ...
  if ((water_level <= low_level) && LvlFlag ==0) { //Low level detected, enter active logic
    LvlFlag = 1;
    int starting_level = getLevel();
    tempMonitor();
    ...
    while (water_level < starting_level + hyst) {  //Delay 7 minutes for water flow to start and level to rise
      ...
      water_level = getLevel();
      //Serial.println("=================="); // Leave commented out: For testing via potentiometer
      delay(dly);  
    }
    
    digitalWrite(Orange_LED, LOW);  //Turn off Orange LED
    water_level2 = getLevel(); //Record level after 5 min pump delay
    
    if (LvlFlag == 1 && errorType == 0) {
      starttime = millis();
      i = 0;
      while (water_level2 <= high_level) { //Future: Includes total timeout time of 18 minutes to stop pump regardless of read level.
        i = timer(20, starttime);
        if (i == 1) {
      ...
      while((water_level2 < low_level + hyst) && LvlFlag == 1 && errorType > 0 && reset_control == "0") {
        digitalWrite(RelayOut_Pin, LOW); //Stop pump
        writeToDB("digital", "D12", 0); // Write pump state to DB
        ...
        //lcd.setCursor(0, 0);
        //lcd.print("Timeout Error:Check & Reset");
        //lcd.setCursor(0, 1);
        //lcd.print("Level=");
        //lcd.print(water_level2);
